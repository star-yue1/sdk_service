...
export class SdkService {
  ...
  // 获取SDK列表
  async getSdkList(query): Promise<{code: number, data: sdk[], total: number}> {
    const { page = 0, size = 10, ...rest } = query
    const temp = await this.sdkTest.find({ ...rest }).sort({create_time: -1}).skip(page * size).limit(size);
  ...
  async updataSdkStatus(query) {
    ...
    if (sdkInfo) {
      const end_time = String(new Date().getTime() + Number(sdkInfo.time) * 86400000)
