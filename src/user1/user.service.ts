...
export class UserService {
  ...
  constructor(
    ...
    @InjectModel('sdk') private sdkTest: Model<SdkDocument>,
  ) {}

  // 验证
  async findOneByUsername(user: string): Promise<UserDocument | undefined> {
    return this.userTest.findOne((item: UserDocument) => item.user === user);
  }
  // 添加
    ...
    if (is_have.length === 0) {
      if (query.sdk) {
        ...
        const end_time = String(
          new Date().getTime() + Number(getSdk.time) * 86400000,
        );
        const user = await this.userTest.create({
          ...query,
